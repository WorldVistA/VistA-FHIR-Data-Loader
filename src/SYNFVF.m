...
getVehuIcns     ; get the Dev VEHU ICNs and stores them in a graph
 n gn,root
 d purgegraph^SYNGRAF("vehu-icns")
 s root=$$setroot^SYNGRAF("vehu-icns")
...
getAllergies    ; get VEHU allergies fhir resources
 ;
 n icnroot s icnroot=$$setroot^SYNGRAF("vehu-icns")
 ;
 d purgegraph^SYNGRAF("fhir-vista-AllergyIntolerance")
 n zicn s zicn=""
...
getAppointments ; get VEHU appointment fhir resources
 ...
 n icnroot s icnroot=$$setroot^SYNGRAF("vehu-icns")
 ;
 d purgegraph^SYNGRAF("fhir-vista-Appointment")
 n zicn s zicn=""
 f  s zicn=$o(@icnroot@("ICN",zicn)) q:zicn=""  d  ;
 . n domain,entries
...
getProcedures   ; get VEHU procedure fhir resources
 ...
 n icnroot s icnroot=$$setroot^SYNGRAF("vehu-icns")
 ;
 d purgegraph^SYNGRAF("fhir-vista-Procedure")
 n zicn s zicn=""
 f  s zicn=$o(@icnroot@("ICN",zicn)) q:zicn=""  d  ;
 . n domain,entries
 . s domain="Procedure"
...
storeGraph(ary,graph,id)        ; stores an array ary, passed by name
 ...
 n root set root=$$setroot^SYNGRAF(graph)
 ;
 n ien,rien
 s ien=$o(@root@(" "),-1)+1
 ;
...
wsGetAllergy(rtn,filter)        ; web service to pull a random allergy from
 ...
 ;
 n root s root=$$setroot^SYNGRAF("fhir-vista-AllergyIntolerance")
 n max
 s max=$o(@root@(" "),-1)
...
wsGetAppointment(rtn,filter)    ; web service to pull a random appointments from
 ...
 ;
 n root s root=$$setroot^SYNGRAF("fhir-vista-Appointment")
 n max
 s max=$o(@root@(" "),-1)
 i +max=0 q  ; nothing to pull
 n ien s ien=$r(max)
...
wsGetProcedure(rtn,filter)      ; web service to pull a random procedure from
 ...
 ;
 n root s root=$$setroot^SYNGRAF("fhir-vista-Procedure")
 n max
 s max=$o(@root@(" "),-1)
 i +max=0 q  ; nothing to pull
 n ien s ien=$r(max)
