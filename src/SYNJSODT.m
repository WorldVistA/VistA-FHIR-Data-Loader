SYNJSODT ;SLC/KCM -- Unit tests for JSON decoding;2019-03-01  10:44 AM
 ;
 d EN^%ut($t(+0),3)
 quit
 ;
...
JSONUES ;; @TEST unescape JSON encoded string
 N X
 S X=$$UESSYNJSON("String with \\ in the middle")
 D ASSERT("String with \ in the middle",X)
 S X=$$UESSYNJSON("\\ is the first character of this string")
 D ASSERT("\ is the first character of this string",X)
 S X=$$UESSYNJSON("The last character of this string is \\")
 D ASSERT("The last character of this string is \",X)
 S X=$$UESSYNJSON("\\one\\two\\three\\")
 D ASSERT("\one\two\three\",X)
 S X=$$UESSYNJSON("A vee shape: \\\/\tTABBED\tand line endings.\n\r\f")
 D ASSERT("A vee shape: \/"_$C(9)_"TABBED"_$C(9)_"and line endings."_$C(10,13,12),X)
 S X=$$UESSYNJSON("\""This text is quoted\""")
 D ASSERT("""This text is quoted""",X)
 S X=$$UESSYNJSON("This text contains an embedded\u001A control character")
 D ASSERT("This text contains an embedded"_$C(26)_" control character",X)
 S X=$$UESSYNJSON("This contains tab\t and control\u0016 characters")
...
SPLITA ;; @TEST JSON input with escaped characters on single line (uses BUILD)
 N JSON,Y,ERR,ESC
 ; V4W/DLW - Removed "string" from SPLIT+3^SYNJSOT1
 S ESC="this contains \and other escaped characters such as "_$c(10)
 ; V4W/DLW - Removed "a piece of" from SPLIT+5^SYNJSOT1
 S ESC=ESC_"  and a few tabs "_$c(9,9,9,9)_" and ""quoted text"""
 D BUILD("SPLIT",.JSON)
 D ASSERT(0,$D(JSON(2)))
 D decode^SYNJSOND("JSON","Y","ERR")
 D ASSERT(0,$D(ERR))
...
VALONLY ;; @TEST passing in value only -- not array
 N JSON,Y,ERR
 S JSON=$P($T(VALONLY+1^SYNJSOT1),";;",2,999)
 D decode^SYNJSOND("JSON","Y","ERR")
 D ASSERT(0,$D(ERR))
...
NUMERIC ;; @TEST passing in numeric types and strings
 N JSON,Y,ERR
 S JSON=$P($T(NUMERIC+1^SYNJSOT1),";;",2,999)
 D decode^SYNJSOND("JSON","Y","ERR")
 D ASSERT(0,$D(ERR))
...
BADQUOTE ;; @TEST poorly formed JSON (missing close quote on LABEL)
 N JSON,Y,ERR
 D BUILD("BADQUOTE",.JSON) D decode^SYNJSOND("JSON","Y","ERR")
 D ASSERT(1,$D(ERR)>0)
 Q
BADSLASH ;; @TEST poorly formed JSON (non-escaped backslash)
 N JSON,Y,ERR
 D BUILD("BADSLASH",.JSON) D decode^SYNJSOND("JSON","Y","ERR")
 D ASSERT(1,$D(ERR)>0)
 Q
...
NUMLABEL ;; @TEST label that begins with numeric
 N JSON,Y,ERR
 D BUILD("NUMLABEL",.JSON) D decode^SYNJSOND("JSON","Y","ERR")
 D ASSERT(0,$D(ERR))
...
ESTRING ;; @TEST a value that looks like an exponents, other numerics
 N JSON,Y,JSON2,ERR
 D BUILD("ESTRING",.JSON) D decode^SYNJSOND("JSON","Y","ERR")
 D ASSERT(0,$D(ERR))
...
SAM1 ;; @TEST decode sample 1 from JSON.ORG
 N JSON,Y,ERR
 D BUILD("SAM1",.JSON) D decode^SYNJSOND("JSON","Y","ERR")
 D ASSERT(0,$D(ERR))
...
SAM2 ;; @TEST decode sample 2 from JSON.ORG
 N JSON,Y,ERR
 D BUILD("SAM2",.JSON) D decode^SYNJSOND("JSON","Y","ERR")
 D ASSERT(0,$D(ERR))
...
SAM3 ;; @TEST decode sample 3 from JSON.ORG
 N JSON,Y,ERR
 D BUILD("SAM3",.JSON) D decode^SYNJSOND("JSON","Y","ERR")
 D ASSERT(0,$D(ERR))
...
SAM4 ;; @TEST decode sample 4 from JSON.ORG
 N JSON,Y,ERR
 D BUILD("SAM4",.JSON) D decode^SYNJSOND("JSON","Y","ERR")
 D ASSERT(0,$D(ERR))
...
SAM5 ;; @TEST decode sample 5 from JSON.ORG
 N JSON,Y,ERR
 D BUILD("SAM5",.JSON) D decode^SYNJSOND("JSON","Y","ERR")
 D ASSERT(0,$D(ERR))
...
MAXNUM ;; @TEST encode large string that looks like number
 ...
 F I=0:1 S X=$P($T(MAXNUM+(I+1)^SYNJSOT1),";;",2,999) Q:X="#####"  S JSON(I)=X
 D decode^SYNJSOND("JSON","Y","ERR")
 D ASSERT(0,$D(ERR))
 D ASSERT(217,$L(Y("taskName","\",1)))
 D encode^SYNJSONE("Y","OUT","ERR")
 D ASSERT(0,$D(ERR))
...
ESCQ ;; @TEST escaped quote across lines
 N JSON,Y,ERR
 D BUILDA("ESCQ",.JSON) D decode^SYNJSOND("JSON","Y","ERR")
 D ASSERT(0,$D(ERR))
 D ASSERT(55,$L(Y("comments")))
 K JSON,Y,ERR
 D BUILDA("ESCQ2",.JSON) D decode^SYNJSOND("JSON","Y","ERR")
 D ASSERT(0,$D(ERR))
 D ASSERT(42,$L(Y("bjw")))
 Q
...
BUILDA(TAG,JSON) ; Build array of string in JSON with splits preserved
 N X,I
 F I=1:1 S X=$E($T(@TAG+I^SYNJSOT1),4,999) Q:X="#####"  S JSON(I)=X
 Q
 ;
