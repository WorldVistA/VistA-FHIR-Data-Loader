SYNJSOET ;SLC/KCM -- Unit tests for JSON encoding;Feb 07, 2019@11:02
 ;
 D EN^%ut($T(+0),3)
 quit
 ;
...
NUMERIC ;; @TEST is numeric function
 D ASSERT(0,$$NUMERIC^SYNJSONE("2COWS"))
 D ASSERT(0,$$NUMERIC^SYNJSONE("007"))
 D ASSERT(0,$$NUMERIC^SYNJSONE(".4"))
 D ASSERT(1,$$NUMERIC^SYNJSONE("0.4"))
 D ASSERT(0,$$NUMERIC^SYNJSONE("-.4"))
 D ASSERT(1,$$NUMERIC^SYNJSONE("-0.4"))
 D ASSERT(1,$$NUMERIC^SYNJSONE(0))
 D ASSERT(0,$$NUMERIC^SYNJSONE(".0"))
 D ASSERT(1,$$NUMERIC^SYNJSONE(3.1416))
 D ASSERT(1,$$NUMERIC^SYNJSONE("2.3E-2"))
 D ASSERT(1,$$NUMERIC^SYNJSONE(0.4E12))
 D ASSERT(0,$$NUMERIC^SYNJSONE(".4E-12"))
 Q
NEARZERO ;; @TEST encode of numbers near 0
 ...
 S X("x")=0.42
 S X("y")=-0.44
 D encode^SYNJSONE("X","JSON","ERR")
 D ASSERT($P($T(NEARZERO+1),";;",2,99),JSON(1))
 Q
JSONESC ;; @TEST create JSON escaped string
 N X
 S X=$$ESCSYNJSON("String with \ in the middle")
 D ASSERT("String with \\ in the middle",X)
 S X=$$ESCSYNJSON("\ is the first character of this string")
 D ASSERT("\\ is the first character of this string",X)
 S X=$$ESCSYNJSON("The last character of this string is \")
 D ASSERT("The last character of this string is \\",X)
 S X=$$ESCSYNJSON("\one\two\three\")
 D ASSERT("\\one\\two\\three\\",X)
 S X=$$ESCSYNJSON("A vee shape: \/"_$C(9)_"TABBED"_$C(9)_"and line endings."_$C(10,13,12))
 D ASSERT("A vee shape: \\\/\tTABBED\tand line endings.\n\r\f",X)
 S X=$$ESCSYNJSON("""This text is quoted""")
 D ASSERT("\""This text is quoted\""",X)
 S X=$$ESCSYNJSON("This text contains an embedded"_$C(26)_" control character")
 D ASSERT("This text contains an embedded\u001A control character",X)
 S X=$$ESCSYNJSON("This contains tab"_$C(9)_" and control"_$C(22)_" characters")
 D ASSERT("This contains tab\t and control\u0016 characters",X)
 S X=$$ESCSYNJSON("This has embedded NUL"_$C(0)_" character.")
...
BUILDY(LABEL) ; build Y array based on LABEL
 ...
 N I,X
 K Y
 F I=1:1 S X=$P($T(@LABEL+I^SYNJSOT2),";;",2,999) Q:X="zzzzz"  X "S "_X
 Q
TARGET(ID,OFFSET) ; values to test against
 S OFFSET=$G(OFFSET,1)
 Q $P($T(@ID+OFFSET^SYNJSOT2),";;",2,999)
 ;
